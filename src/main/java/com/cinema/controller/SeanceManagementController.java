...
public class SeanceManagementController {
    ...
    public SeanceManagementController(SeanceManagementView view) {
        ...
        this.filmDAO = new FilmDAO(); // Pour récupérer les films dans le formulaire
        this.salleDAO = new SalleDAO(); // Pour récupérer les salles dans le formulaire
    ...
    private void setupSearchFilter() {
        ...
        view.getSearchField().textProperty().addListener((observable, oldValue, newValue) -> {
            filteredData.setPredicate(seance -> {
                ...
                // Recherche par titre de film ou par nom de salle
    ...
    // --- LE FORMULAIRE POP-UP POUR LES SÉANCES ---
    private Optional<Seance> showSeanceFormDialog(Seance seanceToEdit) {
        ...
        try { dialogPane.getStylesheets().add(getClass().getResource("/css/style.css").toExternalForm()); } 
        catch (Exception e) {}
        ...
        // 1. Liste déroulante des Films (On affiche le titre, on cache l'objet complet)
        ...
        // Convertisseur pour afficher joliment le titre du film
        filmBox.setConverter(new StringConverter<Film>() {
            @Override public String toString(Film film) { return film != null ? film.getTitre() : ""; }
            @Override public Film fromString(String string) { return null; }
        });

        // 2. Liste déroulante des Salles
        ...
        salleBox.setConverter(new StringConverter<Salle>() {
            @Override public String toString(Salle salle) { return salle != null ? salle.getNomSalle() + " (" + salle.getTechnologie() + ")" : ""; }
            @Override public Salle fromString(String string) { return null; }
        });
        ...
        if (seanceToEdit != null) {
            // On retrouve le film et la salle correspondants dans les listes
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == saveBtnType) {
                try {
                    ...
                    LocalTime heure = LocalTime.parse(heureField.getText()); // Doit être au format HH:mm
